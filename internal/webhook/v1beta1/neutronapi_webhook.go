...
func (v *NeutronAPICustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	if err := neutronapi.Spec.ValidateCreate(basePath, neutronapi.Namespace); err != nil {
		allErrs = append(allErrs, err...)
	}

	if len(allErrs) != 0 {
		return nil, apierrors.NewInvalid(neutronv1beta1.GroupVersion.WithKind("NeutronAPI").GroupKind(), neutronapi.Name, allErrs)
	}
...
func (v *NeutronAPICustomValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	oldNeutronAPI, ok := oldObj.(*neutronv1beta1.NeutronAPI)
	if !ok || oldNeutronAPI == nil {
		return nil, apierrors.NewInternalError(fmt.Errorf("%w", ErrConvertExisting))
	}

	allErrs := field.ErrorList{}
	basePath := field.NewPath("spec")

	if err := neutronapi.Spec.ValidateUpdate(oldNeutronAPI.Spec, basePath, neutronapi.Namespace); err != nil {
		allErrs = append(allErrs, err...)
	}
